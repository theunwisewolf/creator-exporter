...
class ScrollView extends Node {
    ...
    get_content_node() {
        ...
        let view_node = this.get_component();
        if (view_node) {
    ...
    get_component() {
        ...
        if (component_sv === null) {
            component_sv = Node.get_node_component_of_type(this._node_data, 'DynamicScrollView');
        }
    ...
    parse_properties() {
        ...
        // Spriteframe can be null
        if (compent_spr && compent_spr._spriteFrame) {
        ...
        // Should elements be recycled?
        this.add_property_bool('recycleElements', 'recycleElements', component_sv);
