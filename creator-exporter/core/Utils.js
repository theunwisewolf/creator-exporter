...
class Utils {
    ...
    static success(data) {
        const logFunc = Editor ? Editor.success : console.log;
        logFunc(PREFIX + data);
    }

    static warn(data) {
        const logFunc = Editor ? Editor.warn : console.warn;
        logFunc(PREFIX + data);
    }

    static info(data) {
        const logFunc = Editor ? Editor.info : console.log;
        logFunc(PREFIX + data);
    }

    static failed(data) {
        const logFunc = Editor ? Editor.failed : console.log;
        logFunc(PREFIX + data);
    }

    static error(data) {
        const logFunc = Editor ? Editor.error : console.error;
        logFunc(PREFIX + data);
    }
            ...
            list.forEach(function (file) {
                ...
                fs.stat(file, function (err, stat) {
                    if (stat && stat.isDirectory()) {
                        Utils.walkAsync(file, function (err, res) {
    ...
    static walk(dir, done) {
        Utils.walkSync(dir, done);
    }

    static walkSync(dir, done) {
        var results = [];
        fs.readdir(dir, function (err, list) {
            if (err) return done(err);
            var i = 0;
            ...
            (function next() {
                ...
                fs.stat(file, function (err, stat) {
                    if (stat && stat.isDirectory()) {
                        Utils.walkSync(file, function (err, res) {
                            results = results.concat(res);
                            next();
                        });
                    } else {
