...
class WorkerBase {
    ...
    _updateProgress(step) {
        this._progress += step;
        Editor.Ipc.sendToAll('creator-exporter:state-changed',
    ...
    _execTime(tag) {
        ...
        if (this._debug) {
            ...
            Editor.log('[creator-exporter] [' + tag + '] ' + times.toString() + 's');
        }
        this._time = current;
    }
}
...
function registerWorker(workerClass, runEvent) {
    Electron.ipcRenderer.on('creator-exporter:' + runEvent, (event, state, opts) => {
